...
class RunProgram:
    ...
    def run_code_check(self, output_file_name, code_set, out_format, input_file, name_id, profile_id, ir_range, fail_id, sort_request, sort_order):
        ...
        if out_format == '.xlsx':
            ...
            for items in range(len(code_set)):
                ...
                joint_reactions, errors = ecc.GenerateOutputArray(code_check, items, name_id, profile_id, ir_range, fail_id).output_list(sort_request, sort_order)
                sheet_name = 'Load Set ' + str(items + 1)
                sheet = wb.create_sheet(f'{sheet_name}')
                if len(joint_reactions) == 0:
                    self.result_set_errors.append(items + 1)
                ...
                for row, key in enumerate(joint_reactions, start=1):
                    for col, key in enumerate(joint_reactions[row - 1], start=1):
                        sheet.cell(column=col, row=row, value=joint_reactions[row - 1][col - 1])
            del wb['Sheet']
            wb.save(output_file_name)
        else:
            with open(output_file_name, 'w', newline=''):
                for items in range(len(code_set)):
                    code_check, end_index, first_useful_line \
                        = g_code.ParseFileForData(items, input_file, code_set).get_code_check_list_info()
                    joint_reactions, errors = ecc.GenerateOutputArray(code_check, items, name_id, profile_id, ir_range,
                                                                      fail_id).output_list(sort_request, sort_order)

                    with open(output_file_name, 'a', newline='') as a:
                        csv.writer(a).writerows(joint_reactions)
